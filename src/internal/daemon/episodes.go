...
func processAnimeEpisodes(
	...
	if isAnimeMovie(anime) {
		sel.toDownload, magnetsForEpisodes = resolveMovie(configs, anime, animeTitle, sel.toDownload, searcher, customQuery)
	}
...
func resolveMovie(configs *files.Config, anime anilist.MediaList, animeTitle string, episodes []anilist.AiringNode, searcher nyaaSearcher, customQuery string) ([]anilist.AiringNode, map[int]resolvedMagnets) {
	logger.Logger.Info().
		Str("anime", animeTitle).
